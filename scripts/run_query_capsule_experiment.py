"""Compare compact latent query capsules with cold-KV and full-prefill controls."""
...
from flash_cache.query_capsules import run_query_capsules
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--capsule-width", type=int, default=4)
    ...
    if args.capsule_width < 1:
        parser.error("--capsule-width must be at least one")
...
def main() -> None:
    ...
    if args.capsule_width > tokenized.recent_prefix_ids.shape[-1]:
        raise ValueError("Capsule width exceeds the recent query prefix length")
    ...
    capsules, capsule_latency_ms = _timed_cuda(
        lambda: run_query_capsules(
            model,
            tokenizer,
            task,
            tokenized,
    ...
    cold_page_tokens = sum(int(ids.shape[-1]) for ids in tokenized.block_ids)
    summary = {
        "seed": args.seed,
        "model": args.model,
        "gpu": gpu_name,
        ...
        "cold_page_token_count": cold_page_tokens,
        ...
        "query_capsules": {
            "generated_answer": capsules.generated_answer,
            "answer_correct": capsules.answer_correct,
            "online_latency_ms": capsule_latency_ms,
            "capsule_token_count": capsules.capsule_token_count,
            "capsule_width": capsules.capsule_width,
            "refreshed_query_token_count": capsules.refreshed_query_token_count,
            "final_cache_token_count": capsules.final_cache_token_count,
            "final_probe_position": capsules.final_probe_position,
            "capsule_to_raw_page_token_ratio": (
                capsules.capsule_token_count / cold_page_tokens
            ),
        },
        "full_prefill": {
